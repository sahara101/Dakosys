...
const SECTIONS: Section[] = [
  {
    ...
    fields: [
      ...
      { key: "tmdb_api_key", type: "string", description: "TMDB API key. Required by metadata_provider tmdb and tvdb, and by the Next Airing poster view. Under tvdb it supplies show status \u2014 including CANCELLED, which TheTVDB does not distinguish from ended \u2014 while TheTVDB supplies the air times." },
      { key: "tvdb_api_key", type: "string", description: "Your own TheTVDB v4 project API key. Optional — when set, DAKOSYS calls TheTVDB directly instead of going through the DAKOSYS proxy. The TheTVDB dashboard may show a working key as INACTIVE, so DAKOSYS verifies it at startup rather than trusting the badge." },
    ],
  },
  {
    title: "Plex",
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "overlay.overlay_style", type: "background_color | colored_text", default: "background_color", description: "background_color draws the status text on a solid colored bar. colored_text draws the status text in the status color with a transparent background, meant to sit on the gradient enabled by apply_gradient_background. Any other value falls back to background_color with a warning." },
      ...
      { key: "overlay.font_path", type: "string", description: "Full path to the font written into the Kometa overlay YAML. Takes precedence over overlay.font_name, which is otherwise joined onto kometa_config.font_directory." },
      { key: "font_path", type: "string", default: "/app/fonts/Juventus-Fans-Bold.ttf", description: "Font DAKOSYS loads locally to measure text width when sizing overlays. Separate from overlay.font_path, which is what Kometa renders with. Falls back to a bundled font, then to DejaVuSans, if the path does not exist." },
      ...
      { key: "next_airing.provider", type: "trakt | text_file", default: "text_file", description: "Source for the Next Airing Kometa collection. \"trakt\" syncs a Trakt list; \"text_file\" writes a local ordered ID file and requires Kometa 2.3.1 or newer (which needs Python 3.10+)." },
      { key: "next_airing.text_file_path", type: "string", default: "<collections_dir>/next-airing.txt", description: "Where DAKOSYS writes the ordered ID file. Only used when next_airing.provider is text_file." },
      { key: "next_airing.kometa_text_file_path", type: "string", default: "config/<collections dir>/next-airing.txt", description: "Path written into the collection YAML for Kometa to read. Override if Kometa sees the directory at a different path than DAKOSYS does." },
      { key: "metadata_provider", type: "trakt | tmdb | tvdb", default: "tvdb when tmdb_api_key is set, else trakt", description: "Source for show status and next-episode data. \"tvdb\" is recommended and is what setup.py configures for new installs: it needs tmdb_api_key, works without tvdb_api_key by using the DAKOSYS proxy, derives exact per-timezone air times from TheTVDB, and restores the FINAL_EPISODE status. \"tmdb\" needs only tmdb_api_key but has no air time of its own. Either, combined with next_airing.provider: text_file, removes the Trakt requirement entirely. When set to trakt and Trakt denies access (401/403/420/426), the run falls back to TMDB automatically if tmdb_api_key is set." },
      { key: "use_tvmaze", type: "boolean", default: "true", description: "Look up real UTC air times from TVmaze so dates convert correctly per timezone, since TMDB alone provides only a calendar date. Under metadata_provider: tvdb this is a fallback, used only for shows TheTVDB has no air time for; TheTVDB always takes precedence. A show neither source covers keeps its bare TMDB date and is never timezone-converted." },
    ],
  },
  {
